...
export default function LoginPage() {
  ...
  // Forgot password flow: 1 (input), 2 (success)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        const { error: signUpError } = await supabaseClient.auth.signUp({ email, password, options: { data: { full_name: name, phone } } });
  ...
  // Handle forgot password wizard (Magic Link)
  const handleForgotFlow = async (e: FormEvent) => {
    e.preventDefault();
    setError(null);
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Something went wrong during recovery");
    } finally {
      setSubmitting(false);
    }
  };
            ...
            {mode !== "forgot" ? (
              ...
              >
                <div className="flex justify-between items-center border-b border-panel-raised/50 pb-2 mb-3">
                  <span className="text-[9px] text-cyan font-bold uppercase">// RECOVER_PASSWORD</span>
                </div>
                ...
                {forgotStep === 1 ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        ...
                        className="w-1/3 py-2.5 border border-panel-raised text-mist rounded hover:bg-panel-raised/30 transition-all font-mono text-[10px] tracking-widest uppercase"
                      >
                        BACK
                      </button>
                      <button
                        type="submit"
                        disabled={submitting}
                        className="w-2/3 py-2.5 bg-cyan/10 border border-cyan/50 text-cyan rounded hover:bg-cyan/20 transition-all font-mono text-[10px] shadow-glow-cyan tracking-widest uppercase"
                      >
                  ...
                  <div className="space-y-4 text-center py-4">
                    ...
                    <h4 className="font-display text-sm font-bold text-cyan uppercase tracking-wider">Reset Link Sent</h4>
                    <p className="text-[10px] text-mist px-2 leading-relaxed">
                      Check your email and click the link to choose a new password. You may now close this page.
                    </p>
                    <div className="pt-4">
                      <button
                        type="button"
                        ...
                        className="w-full py-2.5 bg-cyan/10 border border-cyan/50 text-cyan rounded hover:bg-cyan/20 transition-all font-mono text-[10px] shadow-glow-cyan tracking-widest uppercase"
                      >
                        BACK TO LOGIN
                      </button>
                    </div>

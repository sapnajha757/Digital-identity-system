...
export default function TimelinePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-14 md:px-10 bg-void text-fog min-h-screen relative overflow-hidden">
      <p className="font-mono text-[11px] uppercase tracking-widest text-magenta">// 02 — TIMELINE RECORD</p>
      <h1 className="mt-3 font-display text-3xl font-bold uppercase tracking-wide">
        <span className="gradient-text">Career Journey</span>
      </h1>
      <p className="mt-3 max-w-xl text-mist text-sm font-sans">
      ...
      {loading && <p className="font-mono text-xs text-mist animate-pulse">// loading chronological registry…</p>}
      {error && (
      ...
      {!loading && !error && events.length === 0 && (
        <div className="rounded-lg border border-dashed border-panel-raised px-6 py-12 text-center">
      ...
      {!loading && !error && eventsByYear.length > 0 && (
        <div className="relative pl-8 ml-4 space-y-12">
          {/* DYNAMIC TIMELINE VERTICAL LINE GROW ANIMATION */}
          <motion.div 
            initial={{ height: 0 }}
            animate={{ height: "100%" }}
            transition={{ duration: 1.2, ease: "easeInOut" }}
            className="absolute left-0 top-2 bottom-2 w-px bg-gradient-to-b from-cyan via-magenta to-panel-raised"
          />
            ...
            return (
              <div key={year} className="relative">
                ...
                <div 
                  ...
                  className="absolute -left-[45px] top-0 flex items-center gap-3 cursor-pointer group select-none z-10"
                >
                  <motion.span 
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="bg-panel border border-cyan/45 hover:border-cyan text-cyan font-display text-xs font-bold px-2.5 py-1 rounded font-mono shadow-glow-cyan/10 transition-all"
                  >
                    {year}
                  </motion.span>
                  <span className="text-[9px] font-mono text-mist/60 group-hover:text-cyan transition-colors">
                        ...
                        return (
                          ...
                          >
                            ...
                            <HudFrame 
                              ...
                              className="bg-panel/40 hover:bg-panel/65 hover:border-cyan/45 transition-all duration-300 shadow-sm rounded-lg border border-panel-raised"
                            >
                              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-panel-raised/40 pb-2.5">
                                <div className="flex flex-wrap items-center gap-2 font-mono text-[9px] uppercase tracking-widest text-mist">
                                  ...
                                  <span 
                                    className={`border px-2 py-0.5 rounded-full font-bold text-[8px] ${
                                  ...
                                  {event.date_inferred && (
                                    <span className="italic text-mist/60 text-[8px]">(Inferred)</span>
                                  )}
                                </div>
                                ...
                                {event.document_id && (
                                  <Link
                                    href={`/graph`}
                                    className="font-mono text-[9px] text-cyan hover:text-magenta transition-colors hover:underline"
                                  >
                                    [VIEW IN GRAPH] →
                                  </Link>
                                )}
                              ...
                              <div className="mt-4 flex items-start gap-4">
                                <div className="w-10 h-10 rounded border border-panel-raised bg-void/50 flex items-center justify-center font-display text-xs font-bold text-mist uppercase shrink-0">
